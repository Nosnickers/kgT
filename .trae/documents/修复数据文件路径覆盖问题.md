...
* 添加 `EmbeddingConfig` 类，包含：
  ...
  * `local_model_path`: 本地模型路径（从 .env 的 LOCAL\_EMBEDDING\_MODEL\_PATH 读取）
...
### 2. 修改 [web\_server.py](file:///d:\workspace\pytorch\KG\web_server.py)
...
* web\_server.py 启动时会自动读取 .env 中的 LOCAL\_EMBEDDING\_MODEL\_PATH
